...
class TestObterResposta(unittest.TestCase):
    ...
    def teste_perguntas_simples(self):
        """Teste de respostas simples - 4 testes"""
    ...
    def teste_despedidas(self):
        """Teste de respostas de despedida - 3 testes"""
    ...
    def teste_programar(self):
        """Teste de respostas sobre programação - 2 testes"""
    ...
    def teste_resposta_padrao(self):
        """Teste de resposta padrão para perguntas não reconhecidas"""
        textos = ["xyz123", "isto não faz sentido", "pergunta estranha", "lalala"]
        for texto in textos:
            self.assertEqual(obter_resposta(texto), f"Desculpa, não entendi a questão! {texto}")
